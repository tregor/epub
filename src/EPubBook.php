...
//        $filename = "./{$this->title}.epub";
        ...
        foreach ($this->chapters as $chapter) {
            $zip->addFromString("OPS/{$chapter['title']}.xml", $this->getChapterXml($chapter));
        }
...
<h3 class="subtitle"><em><span style="font-style:italic;">{$chapter['title']}</span></em></h3>
<p class="ps1">{$chapter['content']}</p>
<p class="br"><br /></p>
HTML;
    ...
    protected function getContentOpf() {
        ...
        $xml->writeElement('dc:title', $this->title);
        $xml->writeElement('dc:creator', $this->author);
        $xml->writeElement('dc:language', $this->language);
        $xml->endElement(); // metadata
        $xml->startElement('manifest');
        // Здесь должны быть элементы манифеста для каждой главы
        foreach ($this->chapters as $chapter) {
            $xml->startElement('item');
            $xml->writeAttribute('id', $chapter['title']);
            $xml->writeAttribute('href', "{$chapter['title']}.xml");
            $xml->writeAttribute('media-type', 'application/xhtml+xml');
            $xml->endElement(); // item
        }
        $xml->endElement(); // manifest
        $xml->startElement('spine');
        // Здесь должны быть элементы spine для каждой главы
        foreach ($this->chapters as $chapter) {
            $xml->startElement('itemref');
            $xml->writeAttribute('idref', $chapter['title']);
    ...
    protected function getTocNcx() {
        ...
        foreach ($this->chapters as $chapter) {
            ...
            $xml->writeAttribute('id', $chapter['title']);
            $xml->writeAttribute('playOrder', $chapter['order']);
            $xml->startElement('navLabel');
            $xml->writeElement('text', $chapter['title']);
            $xml->endElement(); // navLabel
            $xml->startElement('content');
            $xml->writeAttribute('src', "{$chapter['title']}.xml");
